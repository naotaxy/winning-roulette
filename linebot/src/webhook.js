  ...
  if (intent === 'noblesse:rerun') {
    ...
    if (isResearchSummaryRequest(rerunCaseData.request || '') || isResearchSummaryRequest(rerunChosenText || '')) {
      ...
      const researchResult = await callGeminiResearchSummary({
        ...
        gameContext: [
          'ゲーム名: eFootball ウイニングイレブンコレクション（ウイコレ）。KONAMIのカードコレクション型モバイルゲーム。試合はカード戦力で自動計算。リアルタイム操作なし。',
          'リーグ区分: ビギナー→ブロンズ→シルバー→ゴールド→プラチナ→ダイヤモンド→タイタン（最上位）。',
          '重要メカニクス: センス/アドセンス（選手の特性・強さの核）、スキル（カットビジョンが現在流行）、GKスキル（エリアスイーパーが必須級）、新キャラのスキル発動の重さ（無課金では扱いにくい）。',
          'グループ状況: 無課金プレイヤー6人。月次縛りルールのリーグ戦を運営。メンバーの1人が4〜5年かけて初めてタイタンリーグに出場した。',

...
function detectBeastModeIntent(text) {
  ...
  if (/(ビーストモード.*(オフ|解除|やめ|終了)|ビースト解除|ノブレスモード.*(オフ|解除|やめ|終了)|ノブレス解除|ノーマルモード|通常モード|秘書モード)/.test(t)) {
    return { type: 'beastMode', action: 'disable' };
  }

  if (/(ビーストモード|ノブレスモード|beastmode|beastmodeon|beastmodeenable|noblessemode|noblessemodeon)/.test(t)) {
...
function formatBeastModeReply(enabled, action) {
  if (action === 'status') {
    return enabled
      ? [
        'ノブレスモードは今このグループでONだよ。',
        'ノブレス相談が使える状態になってるの。',
        '例: 「@秘書トラペル子 旅行先を決めたい」「@秘書トラペル子 飲み会の店を決めたい」',
      ].join('\n')
      : [
        'ノブレスモードは今OFFだよ。',
        'ノブレス相談を使いたい時は「@秘書トラペル子 ノブレスモード」って呼んでね。',
      ].join('\n');
  }

  if (action === 'disable') {
    return [
      'わかった。このグループのノブレスモードはOFFにしたよ。',
      'またノブレス相談を使いたくなったら「@秘書トラペル子 ノブレスモード」で戻せるからね。',
    ].join('\n');
  }

  return [
    enabled
      ? 'うん、このグループのノブレスモードをONにしたよ。'
      : 'ノブレスモード、起動したよ。',
    '今からノブレス相談が使えるの。',

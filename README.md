...
- ホイール内は番号表示、直下の凡例で正式なルール名を確認
- スピンごとに50%で「入れ歯確変」または「G確変」が発生
- 確変時は候補を12択なら4択、6択なら2択へ圧縮し、専用演出で抽選
...
ルーレット本体、番号付き凡例、SPIN POWERゲージ、抽選済みルール、最終結果を表示します。スピン時には50%で確変モードに入り、候補数を圧縮した専用ホイールに切り替わります。最終結果では保存対象月を選び、カレンダーに登録できます。
